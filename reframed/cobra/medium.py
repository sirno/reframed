...
def minimal_medium(
    ...
    if milp:
        ...
        for r_id in exchange_reactions:
            if direction < 0:
                solver.add_constraint(
                    "c_" + r_id, {r_id: 1, "f_" + r_id: 1}, ">", 0, update=False
                )
            else:
                solver.add_constraint(
                    "c_" + r_id, {r_id: 1, "f_" + r_id: -1}, "<", 0, update=False
    ...
    if min_mass_weight:
        ...
        if multiple_compounds:
            warn_wrapper(
                f"Reactions ignored (multiple compounds): {multiple_compounds}"
            )
        if no_compounds:
    ...
    if direction < 0:
        constraints = {
            r_id: (
                -max_uptake if r_id in valid_reactions else 0,
                model.reactions[r_id].ub,
            )
            for r_id in exchange_reactions
        }
    else:
    ...
    if n_solutions == 1:
        ...
        if solution.status != Status.OPTIMAL:
            ...
            if validate:
                validate_solution(
                    model, medium, exchange_reactions, direction, min_growth, max_uptake
                )
        ...
        for i in range(0, n_solutions):
            if i > 0:
                ...
                solver.add_constraint(
                    constr_id, previous_sol, "<", len(previous_sol) - 1
                )
...
def get_medium(solution, exchange, direction, abstol):
    return set(
        ...
        if (
            direction < 0
            and solution.values[r_id] < -abstol
            or direction > 0
            and solution.values[r_id] > abstol
        )
    )
...
def validate_solution(
    ...
    if direction == -1:
        constraints = {
            r_id: (-max_uptake, inf) if r_id in medium else (0, inf)
            for r_id in exchange_reactions
        }
    else:
        constraints = {
            r_id: (-inf, max_uptake) if r_id in medium else (-inf, 0)
            for r_id in exchange_reactions
        }
